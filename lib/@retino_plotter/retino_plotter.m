classdef retino_plotter < handle
    ...
    methods
			function obj=retino_plotter(obj)
			end %k
			function update(obj)
				rs = obj.cfg.rs;
			...
			function plot_flat(obj)
				...
				rs.lh.h.nodes = plot(rs.lh.flat.vert_full(:,2), rs.lh.flat.vert_full(:,3), '.');
				...
				for i_roi = 1:numel(rois)
					...
					switch hemi
						case 'L'
							...
							x = v(aMap,2);
							y = v(aMap,3);
							h.overlay(i_roi+numel(rois)) = plot(x,y,'.','color', colors(i_roi,:));
							try
								axis equal;
								set(h.overlay(i_roi+numel(rois)), 'Marker', marker_shape{i_roi}, 'LineWidth', line_width(i_roi));
							end
						case 'R'
							%                         aMap=aMap(aMap>src(1).np)-double(src(1).np);
							...
							x = v(aMap,2);
							y = v(aMap,3);
							h.overlay(i_roi) = plot(x,y,'.','color', colors(i_roi,:));
							try
								axis equal;
								set(h.overlay(i_roi), 'Marker', marker_shape{i_roi-numel(rois)/2}, ...
									'LineWidth', line_width(i_roi-numel(rois)/2));
							end
					end
				end
				rs.h.overlay = h.overlay;
