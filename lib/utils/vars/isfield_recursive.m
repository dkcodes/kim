function out = isprop_recursive(varargin)
%allFieldsExist = isprop_recursive(myStructure,'fieldOfTopLevel','fieldOfFirstField','FieldOfThatNextField')
...
%allFieldsExist = isprop_recursive(specimen,'right','directory')
% allFieldsExist =
%      1
...
%allFieldsExist = isprop_recursive(myStructure,'calibration',{'left','right','centre'})
% allFieldsExist =
%      1
%allFieldsExist = isprop_recursive(myStructure,'calibration',{'left','right','centre'},'fc')
% allFieldsExist =
%      1
%allFieldsExist = isprop_recursive(myStructure,'calibration',{'left','right','centre','blah'},'fc')
% allFieldsExist =
%      0
%allFieldsExist = isprop_recursive(myStructure,'calibration',{'left','right','centre'},{'fc','kc'})
% allFieldsExist =
%      0
	...
	if iscell(varargin{2})
		for i=1:length(varargin{2})
			if length(varargin)>2
				out = out && isprop_recursive(varargin{1},varargin{2}{i},varargin{3:end});
			else
				out = out && isprop_recursive(varargin{1},varargin{2}{i});
			end
		end
		if out
		...
		if out && (isprop(varargin{1},varargin{2}) || isfield(varargin{1},varargin{2}))
